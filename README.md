...
기자가 각 매체 사설을 직접 읽고 인용할 구절을 발췌하면, AI는 **그 발췌만을 근거로** 매체 간 논조를 비교하는
초안을 작성합니다. 무엇을 비평할지 고르는 일과 최종 검토·발행은 항상 기자와 데스크가 합니다.
...
| 하지 않는 것 | 하는 것 |
|---|---|
| 기사 본문 수집·저장 (크롤러·군집·매칭 엔진 제거) | 네이버 검색 API 통과 조회 — 무변형 노출, 우리 판단(매체명 등)은 별도 배열로 전달 |
